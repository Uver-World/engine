...
use client_profile::models::{color::Color, direction::Direction, shape::Shape};
...
impl Filter {
    ...
    pub fn toggle_group_filter(&mut self, group_filter: String, entities: &mut Query<(&DisplayEntity, &mut Visibility)>) {
        self.toggle_group(group_filter);
        self.update_filter(entities);
    }
    
    pub fn toggle_shape_filter(&mut self, shape_filter: Shape, entities: &mut Query<(&DisplayEntity, &mut Visibility)>) {
        self.toggle_shape(shape_filter);
        self.update_filter(entities);
    }

    pub fn toggle_direction_filter(&mut self, direction_filter: Direction, entities: &mut Query<(&DisplayEntity, &mut Visibility)>) {
    ...
    fn toggle_direction(&mut self, direction_filter: Direction) {
        if !self.toggled_direction_filters.contains(&direction_filter) {
            ...
            self.toggled_direction_filters.retain(|direction| direction != &direction_filter)
        }
    }
    ...
    fn update_filter(&self, entities: &mut Query<(&DisplayEntity, &mut Visibility)>) {
        if self.toggled_color_filters.is_empty() && self.toggled_group_filters.is_empty() && self.toggled_shape_filters.is_empty() && self.toggled_direction_filters.is_empty(){
            return Self::make_all_entities_visible(entities);
        }
        
        for (display_entity, mut visibility) in entities {
            if self.toggled_color_filters.contains(&display_entity.settings.group.color) || self.toggled_group_filters.contains(&display_entity.settings.group.group) || self.toggled_shape_filters.contains(&display_entity.settings.group.shape) || self.has_direction(display_entity) {
                *visibility = Visibility::Visible
            } else {
